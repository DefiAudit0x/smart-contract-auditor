#!/usr/bin/env python3
"""Record stage-by-stage observations for the quarantined Nomad candidate."""
...
from benchmarks.real_world.owned_reproductions.nomad.nomad_invariant import evaluate as evaluate_nomad_invariant
...
def run(model: str) -> dict:
    ...
    vulnerable_invariant = evaluate_nomad_invariant(VULNERABLE)
    fixed_invariant = evaluate_nomad_invariant(FIXED)
    poc = run_foundry_poc(POC, ROOT)

    comparator = {
        "status": "NotApplicable",
        "expected_detectors": [],
        "reason": "The existing detector taxonomy has no semantically valid mapping for Nomad zero-root message-proof validation.",
    }
    static_plus_llm = {
        "status": "ObservationalOnly",
        "reason": "No expected detector is assigned; predictions are recorded but cannot create Ground Truth.",
    ...
    full_pipeline = {
        ...
        "reason": "Source mapping and owned reproduction are present, but independent block verification, second adjudication, and detector-family decision remain open.",
    }
    return {
        "schema_version": 1,
        "run_id": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
        "case_id": "rw-003-nomad-bridge",
        ...
        "stages": {
            ...
            "invariant": {
                "id": "nomad.zero_root_not_accepted",
                "vulnerable": {
