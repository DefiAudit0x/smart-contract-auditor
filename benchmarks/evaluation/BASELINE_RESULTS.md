...
The initial POC ran the existing detector methods without compiler-version branching and left `verification/comparator.py` unchanged. Historical detector findings were produced but remained `Rejected` by the unchanged source-vocabulary Comparator, while modern findings were `Confirmed`. A broken old-AST container returned `ASTNormalizationFailed` with provenance instead of silently becoming zero findings. Its original 5-test artifact remains preserved in [`poc_results.json`](../historical_compatibility/canonical_ast_poc/metadata/poc_results.json).

The Revised Compatibility POC then added context-safe semantic mapping, strict legacy/modern schema ownership, AST-native structural checks for function-like declarations and source ranges, explicit status-bearing analysis results, finding-level provenance, a DetectorInput contract, and one standard-json two-file import slice. It includes historical and modern identifier/string/comment negative controls, all declared failure states including `Inconclusive`, and 13 focused regression tests after the imported-source Gate 1 extension. The revised output is kept separately in [`REVISED_POC_REPORT.md`](../historical_compatibility/canonical_ast_poc/REVISED_POC_REPORT.md), [`revised_poc_results.json`](../historical_compatibility/canonical_ast_poc/metadata/revised_poc_results.json), and the Gate 1 evidence in [`GATE1_IMPORTED_SOURCE_REPORT.md`](../historical_compatibility/architecture_boundary/GATE1_IMPORTED_SOURCE_REPORT.md).

Gate 1 is now passed within the isolated POC: `Main.sol` imports `Lib.sol`, the vulnerability exists only in `Lib.sol`, the source-scoped detector emits a finding with `source_id = Lib.sol` and a `Lib.sol:` canonical expression ID, and the unchanged Comparator confirms evidence at `Lib.sol` line 5. Gate 2 then passed as an isolated deterministic policy validation: explicit/verified versions, multiple-candidate ambiguity, pragma conflict, unavailable compiler, no-pragma policy, and post-resolution compilation failure all produce interpretable outcomes with no silent fallback. Gate 3 passed as an isolated provenance/retention contract: source, manifest, raw AST, Canonical summary, finding payload, and compiler/adapter identity are persisted content-addressed, with replay verification and tamper detection. Production resolver and artifact-store implementations remain unstarted. The full suite remains green at 340 passed; `analyzers/` and `verification/comparator.py` remain unchanged. The Primary Benchmark remains 1.0/1.0/1.0, Nomad/BonqDAO/Parity remain Quarantined, Parity is not re-adjudicated, and no Case #4 was added. See [`GATE2_COMPILER_RESOLUTION_REPORT.md`](../historical_compatibility/architecture_boundary/GATE2_COMPILER_RESOLUTION_REPORT.md) and [`GATE3_PROVENANCE_RETENTION_REPORT.md`](../historical_compatibility/architecture_boundary/GATE3_PROVENANCE_RETENTION_REPORT.md).
