...
The Revised Compatibility POC then added context-safe semantic mapping, strict legacy/modern schema ownership, AST-native structural checks for function-like declarations and source ranges, explicit status-bearing analysis results, finding-level provenance, a DetectorInput contract, and one standard-json two-file import slice. It includes historical and modern identifier/string/comment negative controls, all declared failure states including `Inconclusive`, and 12 focused regression tests. The revised output is kept separately in [`REVISED_POC_REPORT.md`](../historical_compatibility/canonical_ast_poc/REVISED_POC_REPORT.md) and [`revised_poc_results.json`](../historical_compatibility/canonical_ast_poc/metadata/revised_poc_results.json).

The revised gates passed within the POC scope, but this is authorization to prepare a Production Architecture Proposal—not a production merge or detector migration. The full suite remains green at 334 passed; `analyzers/` and `verification/comparator.py` remain unchanged. The Primary Benchmark remains 1.0/1.0/1.0, Nomad/BonqDAO/Parity remain Quarantined, Parity is not re-adjudicated, and no Case #4 was added.
