...
### Real-World corpus and negative controls

The Real-World track is separate from the ten-case Synthetic Ground Truth Benchmark. The current registry contains ten candidates discovered from publicly documented incidents and source PoCs. Each candidate is pinned to a discovery-repository commit and source SHA-256 where available, but all ten remain quarantined because complete source-version mapping, exact affected line ranges, independent reproduction, and adjudication are not yet complete. These candidates therefore contribute **zero** to primary TP, FP, FN, TN, Precision, Recall, or F1.

The candidate registry and admission policy live in [`benchmarks/real_world/registry.json`](benchmarks/real_world/registry.json) and [`benchmarks/real_world/README.md`](benchmarks/real_world/README.md). Each candidate has a corresponding quarantined adjudication record. The adjudication contract explicitly separates external incident evidence from analyzer and LLM observations. The Real-World-inspired negative-control suite contains five bounded, repository-owned safe fixtures and currently evaluates fifteen expected absence checks with zero unexpected detector matches. This is a supplementary false-positive measurement, not a production-protocol safety claim.

The isolated baseline report is generated by `benchmarks/run_track_baseline.py`. It reports Synthetic primary metrics, Synthetic supplementary controls and variants, Real-World negative controls, Real-World candidate status, and the LLM comparison without merging their denominators. The current recorded LLM run used `gpt-5-mini` on ten controls and five attack variants and returned seven true positives, three false positives, and zero false negatives; its Precision was `0.7` and Recall was `1.0`. The deterministic comparison on the same expected assertions returned seven true positives, zero false positives, and zero false negatives. Because the LLM result is model- and run-dependent, it remains descriptive evidence rather than a CI gate.
...
[9]: benchmarks/run_track_baseline.py "Isolated cross-track baseline runner"

[10]: benchmarks/real_world/README.md "Real-World corpus provenance and admission policy"

[11]: benchmarks/evaluation/BASELINE_RESULTS.md "Recorded cross-track baseline results"
